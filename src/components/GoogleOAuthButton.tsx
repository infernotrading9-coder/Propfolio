import React, { useEffect, useRef } from 'react';

interface GoogleOAuthButtonProps {
  onSuccess: (credential: string) => void;
  onError: () => void;
  text?: string;
  disabled?: boolean;
}

// Extend the global Window interface to include google
declare global {
  interface Window {
    google?: {
      accounts: {
        id: {
          initialize: (config: any) => void;
          renderButton: (element: HTMLElement, config: any) => void;
          prompt: () => void;
        };
      };
    };
  }
}

export const GoogleOAuthButton: React.FC<GoogleOAuthButtonProps> = ({
  onSuccess,
  onError,
  text = "Continue with Google",
  disabled = false
}) => {
  const buttonRef = useRef<HTMLDivElement>(null);
  const isInitialized = useRef(false);
  const [showFallback, setShowFallback] = React.useState(false);

  useEffect(() => {
    const timer = setTimeout(() => {
      if (!isInitialized.current) {
        setShowFallback(true);
      }
    }, 3000); // Show fallback after 3 seconds if Google SDK hasn't loaded

    // Load Google Identity Services script
    if (!document.querySelector('script[src*="accounts.google.com"]')) {
      const script = document.createElement('script');
      script.src = 'https://accounts.google.com/gsi/client';
      script.async = true;
      script.defer = true;
      script.onload = () => {
        clearTimeout(timer);
        initializeGoogleAuth();
      };
      script.onerror = () => {
        clearTimeout(timer);
        setShowFallback(true);
      };
      document.head.appendChild(script);
    } else if (window.google && !isInitialized.current) {
      clearTimeout(timer);
      initializeGoogleAuth();
    }

    return () => clearTimeout(timer);
  }, []);

  const initializeGoogleAuth = () => {
    if (!window.google || !buttonRef.current || isInitialized.current) return;

    try {
      console.log('Initializing Google OAuth...');
      
      window.google.accounts.id.initialize({
        client_id: import.meta.env.VITE_GOOGLE_CLIENT_ID,
        callback: handleCredentialResponse,
        auto_select: false,
        cancel_on_tap_outside: true,
        use_fedcm_for_prompt: false, // Helps with COOP issues
        ux_mode: 'popup', // Use popup mode instead of redirect
        context: 'signin',
        itp_support: true, // Enable Intelligent Tracking Prevention support
      });

      window.google.accounts.id.renderButton(buttonRef.current, {
        theme: 'outline',
        size: 'large',
        width: '100%',
        text: 'continue_with',
        shape: 'rectangular',
        logo_alignment: 'left',
      });

      isInitialized.current = true;
      setShowFallback(false); // Hide fallback when Google button is successfully initialized
      console.log('Google OAuth initialized successfully');
    } catch (error) {
      console.error('Google OAuth initialization error:', error);
      
      // Check if it's a COOP-related error
      if (error instanceof Error && error.message.includes('postMessage')) {
        console.warn('COOP policy blocking Google OAuth - showing fallback');
      }
      
      setShowFallback(true); // Show fallback if initialization fails
    }
  };

  const handleCredentialResponse = (response: any) => {
    try {
      if (response.credential) {
        onSuccess(response.credential);
      } else {
        console.log('No credential received from Google OAuth');
        onError();
      }
    } catch (error) {
      console.error('Error handling Google OAuth response:', error);
      onError();
    }
  };

  // Fallback button if Google SDK fails to load
  const handleFallbackClick = () => {
    if (!import.meta.env.VITE_GOOGLE_CLIENT_ID) {
      alert('Google OAuth is not configured. Please add VITE_GOOGLE_CLIENT_ID to your environment variables.');
      return;
    }
    
    // Manual OAuth flow as fallback
    const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID;
    const redirectUri = encodeURIComponent(window.location.origin + '/auth/google/callback');
    const scope = encodeURIComponent('openid email profile');
    const responseType = 'code';
    
    const authUrl = `https://accounts.google.com/oauth/authorize?` +
      `client_id=${clientId}&` +
      `redirect_uri=${redirectUri}&` +
      `scope=${scope}&` +
      `response_type=${responseType}&` +
      `prompt=select_account`;
    
    window.location.href = authUrl;
  };

  return (
    <div className="w-full">
      {!showFallback && (
        <div
          ref={buttonRef}
          className={`w-full min-h-[44px] ${disabled ? 'pointer-events-none opacity-50' : ''}`}
        />
      )}
      
      {/* Fallback button */}
      {showFallback && (
        <div className="w-full space-y-2">
          <button
            onClick={handleFallbackClick}
            disabled={disabled}
            className={`w-full px-4 py-3 border border-white/20 rounded-lg text-white/90 hover:text-white hover:bg-white/5 transition-all duration-300 flex items-center justify-center gap-3 font-medium ${disabled ? 'pointer-events-none opacity-50' : ''}`}
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path
                fill="#4285f4"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              />
              <path
                fill="#34a853"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              />
              <path
                fill="#fbbc05"
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              />
              <path
                fill="#ea4335"
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              />
            </svg>
            {text}
          </button>
          <p className="text-xs text-white/50 text-center">
            Note: Using alternative OAuth flow due to browser security settings
          </p>
        </div>
      )}
    </div>
  );
};